...
def preprocess(
    ...
    for conversation, target in zip(conversations, targets):
        ...
        for i, turn in enumerate(turns):
            ...
            # # "-1" is hardcoded for the Qwen3 tokenizer to make the offset correct.
...
def preprocess_internlm(
    ...
    for conversation, target in zip(conversations, targets):
        ...
        #         # Thêm dòng này:
        #         print(f'  conversation[:200] = {repr(conversation[:200])}')
        #         sys.stdout.flush()
        if cur_len < tokenizer.model_max_length:
            logger.warning(f'tokenization mismatch: {cur_len} vs. {total_len}. Dataset: {ds_name}.')
            if abs(cur_len - total_len) > 3:  # tolerance 3 tokens thay vì == 0
                target[:] = IGNORE_TOKEN_ID
                print(f'WARNING: tokenization mismatch: {cur_len} vs. {total_len}. Dataset: {ds_name}.')
                sys.stdout.flush()
                # Nếu lệch nhỏ (1-3 tokens) → bỏ qua, labels vẫn hợp lệ
